# Generated by Django 5.1.3 on 2024-11-21 14:38

import django.db.models.deletion
import login.views
from django.conf import settings
from django.db import migrations, models


class Migration(migrations.Migration):

    initial = True

    dependencies = [
        migrations.swappable_dependency(settings.AUTH_USER_MODEL),
    ]

    operations = [
        migrations.CreateModel(
            name='Match',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('date', models.DateField(auto_now_add=True)),
                ('player_one_pts', models.IntegerField(default=0)),
                ('player_two_pts', models.IntegerField(default=0)),
                ('exchanges', models.IntegerField(default=0)),
                ('exchangesMax', models.IntegerField(default=0)),
                ('player_one_goals_up', models.IntegerField(default=0)),
                ('player_two_goals_up', models.IntegerField(default=0)),
                ('player_one_goals_mid', models.IntegerField(default=0)),
                ('player_two_goals_mid', models.IntegerField(default=0)),
                ('player_one_goals_down', models.IntegerField(default=0)),
                ('player_two_goals_down', models.IntegerField(default=0)),
                ('player_one', models.ForeignKey(on_delete=models.SET(login.views.create_nobody), related_name='first_player', to=settings.AUTH_USER_MODEL)),
                ('player_two', models.ForeignKey(on_delete=models.SET(login.views.create_nobody), related_name='second_player', to=settings.AUTH_USER_MODEL)),
                ('winner', models.ForeignKey(on_delete=models.SET(login.views.create_nobody), related_name='matches_won', to=settings.AUTH_USER_MODEL)),
            ],
        ),
        migrations.CreateModel(
            name='TournamentMatch',
            fields=[
                ('match_ptr', models.OneToOneField(auto_created=True, on_delete=django.db.models.deletion.CASCADE, parent_link=True, primary_key=True, serialize=False, to='game.match')),
                ('round', models.IntegerField(default=0)),
                ('match', models.IntegerField(default=0)),
            ],
            bases=('game.match',),
        ),
        migrations.CreateModel(
            name='TournamentModel',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('date', models.DateField(auto_now_add=True)),
                ('winner', models.ForeignKey(on_delete=models.SET(login.views.create_nobody), related_name='tournaments_won', to=settings.AUTH_USER_MODEL)),
                ('matches', models.ManyToManyField(related_name='matches', to='game.tournamentmatch')),
            ],
        ),
    ]
